...
# =============================
# Парсинг часов работы (упрощённо)
# =============================
...
@dataclass
class HoursWindow:
    days: Optional[List[int]]  # None => ежедневно
    start: Optional[time]      # None => круглосуточно
    end: Optional[time]
...
def parse_working_hours(text: str) -> Optional[List[HoursWindow]]:
    ...
    # Дни недели, например: Пн-Пт: с 09:00 до 18:00; Сб-Вс: с 10:00 до 22:00
    # Разобьём на сегменты по ";"
...
def find_relevant_places(
    ...
    for score, ix in zip(sim_scores.tolist(), idxs.tolist()):
        ...
        # Открыто ли сейчас: используем как небольшой бонус/штраф
    ...
    # Сортируем по финальному скору и возвращаем топ-N
...
if __name__ == "__main__":
    # Небольшой smoke-test (ожидает, что артефакты уже созданы data_processor.py)
    try:
        res = find_relevant_places(
            user_query="тихое кафе с wi-fi",
            user_location=(51.1694, 71.4491),  # район Байтерека
            max_distance_km=3.0,
            max_results=5,
        )
        print(json.dumps(res, ensure_ascii=False, indent=2))
    except Exception as e:
        print("Ошибка выполнения retrieval:", e)
