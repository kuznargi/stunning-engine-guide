...
load_dotenv()  # load API keys, etc.

app = FastAPI(title="Kazakhstan Guide AI API", version="0.2.0")

# CORS for Vite dev server and common local hosts
origins = [
    "http://localhost:5173",
    "http://127.0.0.1:5173",
    "http://localhost:3000",
    "http://127.0.0.1:3000",
    "*",  # relax for now; tighten in production
]
app.add_middleware(
    CORSMiddleware,
...
def load_processed_pois() -> List[Dict[str, Any]]:
    """Load and cache processed POIs from JSON file.

    Returns list of all POI records from processed_pois.json.
...
def filter_kazakhstan_locations(
    ...
    """Filter POIs to get interesting locations in Kazakhstan.

    Args:
        pois: List of all POI records
        city: Optional city filter (e.g., "Алматы", "Астана", "Актобе")
        region: Optional region filter (e.g., "Алматинская область")
        min_popularity: Minimum popularity score (0-1)
        limit: Maximum number of locations to return

    Returns:
        Filtered and sorted list of locations
    """
    # Kazakhstan approximate bounding box
    # Lat: 40.5 to 55.5, Lon: 46.5 to 87.5
    KZ_LAT_MIN, KZ_LAT_MAX = 40.5, 55.5
    KZ_LON_MIN, KZ_LON_MAX = 46.5, 87.5

    # Major cities bounding boxes for faster filtering
    CITY_BOUNDS = {
...
def cities() -> Dict[str, Any]:
    """
    Get list of major cities and regions in Kazakhstan.

    Returns available cities with their metadata for filtering locations.
    """
    major_cities = [
...
def locations(
    ...
    """
    Dynamic popular locations in Kazakhstan from processed_pois.json.

    Returns top locations sorted by quality score (popularity + data completeness).
    Locations are cached for performance.

    Query parameters:
    - city: City name (e.g., "Алматы", "Астана")
    - region: Region name (e.g., "Алматинская область")
    - limit: Max number of locations (default 50)
    - min_popularity: Minimum popularity score 0-1 (default 0.3)
    - category: Optional category filter (e.g. "кафе", "парк", "музей")
    - include_city_center: Add city center as first location (default: True)
    """
    try:
        # City center mapping
        CITY_CENTERS = {
        ...
        # Load all POIs (cached)
        all_pois = load_processed_pois()

        # Filter to Kazakhstan with quality criteria
        ...
        # Add city center as first location if requested and city is specified
        ...
        # Clean up response (remove quality_score, keep only essentials)
